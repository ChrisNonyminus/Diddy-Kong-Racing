...
void load_level(s32 levelId, s32 numberOfPlayers, s32 entranceId, s32 vehicleId, s32 cutsceneId) {
    ...
    if (func_8006C2F0() == 0) {
        if (D_800DD32C == 0) {
            ...
            if (gCurrentLevelHeader->race_type == RACETYPE_HUB_AREA) {
            ...
            if (gCurrentLevelHeader->race_type == RACETYPE_HUB_AREA && gCurrentLevelHeader->world == 0 && !(settings->cutsceneFlags & 0x2000) && settings->wizpigAmulet >= 4) {
